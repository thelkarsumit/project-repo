...
resource "azurerm_monitor_metric_alert" "data_disk_alert_ci" {
  count = length(var.severity_level)
  name = "Alert-AM-Prod-Subscription-Windows-DataDiskIOPS-${var.severity_level[count.index]}-CentralIndia"
