resource "azurerm_monitor_scheduled_query_rules_alert_v2" "memory_bytes_percentage_alert" {
  ...
  name                = "Alert-AM-Hub-Subscription-AKS-Container-Memory-Percentage-${var.severity_level[count.index]}"
  ...
  display_name                     = "Alert-AM-Hub-Subscription-AKS-Container-Memory-Percentage-${var.severity_level[count.index]}"
  enabled                          = true
