resource "azurerm_monitor_scheduled_query_rules_alert_v2" "memory_used_swap_space_alert" {
  ...
  name                = "Alert-AM-Hub-Subscription-linux-MemoryUsedSwapSpace-${var.severity_level[count.index]}"
...
| project TimeGenerated, Computer, CounterValue, _ResourceId     
  ...
  display_name                     = "Alert-AM-Hub-Subscription-linux-MemoryUsedSwapSpace-${var.severity_level[count.index]}"
  enabled                          = true
