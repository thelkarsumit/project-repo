resource "azurerm_monitor_scheduled_query_rules_alert_v2" "logical_disk_used_space_alert" {
  ...
  name                = "Alert-AM-Hub-Subscription-linux-LogicalDiskUsedSpace-${var.severity_level[count.index]}"
...
| summarize AggregatedValue = avg(CounterValue) by bin(TimeGenerated, 5m), Computer, InstanceName, _ResourceId
  ...
  display_name                     = "Alert-AM-Hub-Subscription-linux-LogicalDiskUsedSpace-${var.severity_level[count.index]}"
  enabled                          = true
