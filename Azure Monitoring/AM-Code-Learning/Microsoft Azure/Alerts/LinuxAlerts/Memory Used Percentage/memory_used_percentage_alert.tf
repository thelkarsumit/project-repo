resource "azurerm_monitor_scheduled_query_rules_alert_v2" "alert_v2" {
  ...
  name                = "Alert-AM-Hub-Subscription-linux-MemoryUsedPercentage-${var.severity_level[count.index]}"
  ...
  display_name                     = "Alert-AM-Hub-Subscription-linux-MemoryUsedPercentage-${var.severity_level[count.index]}"
  enabled                          = true
