resource "azurerm_monitor_scheduled_query_rules_alert_v2" "CPUPercentage-container" {
  ...
  name                = "Alert-AM-Hub-Subscription-AKS-Container-CPU-Percentage-${var.severity_level[count.index]}"
...
| project  UsagePercent, _ResourceId, CounterName, Computer
  ...
  display_name                     = "Alert-AM-Hub-Subscription-AKS-Container-CPU-Percentage-${var.severity_level[count.index]}"
  enabled                          = true
