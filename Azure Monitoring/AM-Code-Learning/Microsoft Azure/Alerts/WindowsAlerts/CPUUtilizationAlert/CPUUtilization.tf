resource "azurerm_monitor_scheduled_query_rules_alert_v2" "cpu-alert-windows" {
  ...
  name                = "Alert-AM-Hub-Subscription-Windows-CpuUtilization-${var.severity_level[count.index]}"
  ...
  display_name                     = "Alert-AM-Hub-Subscription-Windows-CpuUtilization-${var.severity_level[count.index]}"
  enabled                          = true
