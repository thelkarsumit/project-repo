resource "azurerm_monitor_metric_alert" "DBC-PostgreSQL-Availability" {
  for_each = var.metric_alert
  name = "Alert-AM-Hub-Subscription-PostgreSQL-FlexibleServer-${each.key}-DatabaseIsAlive-Critical"
