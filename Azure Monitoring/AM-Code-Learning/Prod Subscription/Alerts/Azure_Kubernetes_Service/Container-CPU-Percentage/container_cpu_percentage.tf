resource "azurerm_monitor_scheduled_query_rules_alert_v2" "CPUPercentage-container" {
  ...
  name                = "Alert-AM-Prod-Subscription-AKS-Container-CPU-Percentage-${var.severity_level[count.index]}"
...
QUERY
  ...
  display_name                     = "Alert-AM-Prod-Subscription-AKS-CPU-Percentage-${var.severity_level[count.index]}"
  enabled                          = true
