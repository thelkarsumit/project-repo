locals {
  ...
  os_types = [for machine in var.virtual_machines_westeurope.vms : machine["os_type"]]
}
...
resource "azurerm_monitor_metric_alert" "data_disk_alert_we" {
  count = length(var.severity_level)
  name = "Alert-AM-Prod-Subscription-Windows-DataDiskIOPS-${var.severity_level[count.index]}-WestEurope"
