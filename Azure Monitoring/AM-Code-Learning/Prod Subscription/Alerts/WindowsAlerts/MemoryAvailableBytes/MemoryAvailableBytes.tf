...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "memory_available_bytes_alert" {
  ...
  name                = "Alert-AM-Prod-Subscription-Windows-MemoryAvailable-${var.severity_level[count.index]}"
  ...
  display_name                     = "Alert-AM-Prod-Subscription-Windows-MemoryAvailable-${var.severity_level[count.index]}"
  enabled                          = true
