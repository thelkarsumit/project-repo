...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "logical_disk_free_space_alert" {
  ...
  name                = "Alert-AM-Dev-Subscription-Windows-LogicalDiskFreeSpace-${var.severity_level[count.index]}"
  ...
  display_name                     = "Alert-AM-Dev-Subscription-Windows-LogicalDiskFreeSpace-${var.severity_level[count.index]}"
  enabled                          = true
